...
## Git pull/push 시 Password 없이 깃허브에 올리고 싶다면...
...
credential.helper의 store 옵션을 주게되면 해당 git directory에선 반영구적으로 인증 절차가 생략된다.
...
시간은 timeout 옵션으로 지정 가능하다. (초 단위이며 위와 같이 지정 시 3600초, 즉 1시간의 유효시간)
...
git config의 공통적인 설정과 같이 --global 옵션을 주게되면 해당 git directory 이외에 모든 git 활동에서 저장된 정보를 이용하게 된다.
